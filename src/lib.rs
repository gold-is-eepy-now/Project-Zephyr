...
pub fn render_page_text(html: &str, wrap_width: usize) -> String {
    let engine_doc = parse_html_spec(html);

    fn has_ancestor(engine_doc: &EngineDocument, mut node_id: usize, tag: &str) -> bool {
        while let Some(parent) = engine_doc.nodes[node_id].parent {
            if engine_doc.nodes[parent].tag.as_deref() == Some(tag) {
                return true;
            }
            node_id = parent;
        }
    ...
    fn in_visible_tree(engine_doc: &EngineDocument, mut node_id: usize) -> bool {
        let hidden_tags = ["script", "style", "noscript", "template", "head", "meta", "link"];
        while let Some(parent) = engine_doc.nodes[node_id].parent {
            if let Some(tag) = &engine_doc.nodes[parent].tag {
                if hidden_tags.contains(&tag.as_str()) {
                    return false;
                }
            }
            node_id = parent;
    ...
    for node in &engine_doc.nodes {
        if node.tag.is_none() {
            let t = node.text.trim();
            if !t.is_empty() && in_visible_tree(&engine_doc, node.id) {
                if has_ancestor(&engine_doc, node.id, "body") || has_ancestor(&engine_doc, node.id, "#document") {
